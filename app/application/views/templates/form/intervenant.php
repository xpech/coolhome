	...
	if (!$intervenant) $intervenant = new CIntervenant();
	$refid = uniqid('form_');
	?>
	<div class="form-group <? if (isset($class)) echo $class; ?>" allow-new="new">
		...
		<div class="input-group">
			...
			<div class="input-group-append">
				<button class="btn btn-secondary" type="button" role="SelectPopup" url="/intervenants/popup"  target-text="#<?= $refid ?>_txt"  title="Selectionner <?= $title ?>" target-val="#<?= $refid ?>"><i class="fas fa-search"></i></button>
				<? if ($intervenant->id):?>
