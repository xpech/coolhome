...
//-----------------------------------------------------------------------------
	...
	var $causeBlocage;		// Cause du blocage
	var $nEchecConnexion;	// Nombre d'echec de connexion
	var $id = 0;			// id en base
...
//-----------------------------------------------------------------------------
	...
	static public function objectWithEmail($email)
	{
		return self::query('SELECT * FROM t_user WHERE email = %s',CDatabase::DB()->STRING($email));
	}
...
//-----------------------------------------------------------------------------
	...
	{
		$candidates = self::query("SELECT * FROM t_utilisateurs WHERE  e_mail = %s AND actif = 1",

		CDatabase::DB()->STRING($login),CDatabase::DB()->STRING($login));
	...
	function changePassword($new_password)
	{
		...
		$query = sprintf("UPDATE t_utilisateurs set `password` = %s WHERE id = %d ",
